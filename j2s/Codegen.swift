...
public enum JSON {
	init?(value: Any) {
		if let number = value as? NSNumber {
			...
			default: fatalError("unknown CFType \(CFNumberGetType(number))")
			}
			...
			if let date = ISO8601DateFormatter().date(from: string) {
				...
				self = .uuid(uuid)
			} else if string.starts(with: "http://") || string.starts(with: "https://"),
...
extension JSONDecoder.DateDecodingStrategy: Hashable {
	public func hash(into hasher: inout Hasher) {
		...
		case .formatted(let f): hasher.combine(f)
		case .deferredToDate: hasher.combine(1)
		case .iso8601: hasher.combine(8601)
		case .millisecondsSince1970: hasher.combine(10)
		case .secondsSince1970: hasher.combine(100)
		default: fatalError()
		}
	}
}
