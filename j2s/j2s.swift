...
public struct Struct: CustomStringConvertible {
    ...
    private var initDeclarationCode: String {
        let sorted = properties.sorted(by: { x, y in return x.name < y.name })
        return sorted.map({ return "\($0.name): \($0.type))" }).joined(separator: ", ")
    }
    ...
    private var initInitializationCode: String {
        ...
        let lets: [String] = sorted.map({
            if $0.isArray {
                ...
                let a = $0.isOptional ? "as?" : "as!"
                var letString = "\n\t\tlet \($0.name.camelCased()) = dictionary[\"\($0.name)\"] \(a) [[String: Any]]"
                letString += "\n\t\tself.\($0.name.camelCased()) = \($0.name.camelCased()).flatMap { return \($0.name.generatedClassName())($0) }"
                return letString
            } else if !$0.internetPrimitive {
                let a = $0.isOptional ? "as?" : "as!"
                return "self.\($0.name.camelCased()) = \($0.type)(dictionary[\"\($0.name)\"] \(a) [String: Any])"
            } else {
                let a = $0.isOptional ? "as?" : "as!"
                return "self.\($0.name.camelCased()) = dictionary[\"\($0.name)\"] \(a) \($0.type)"
            }
...
public struct Property: Equatable, Hashable {
    ...
    init(name: String, type: String, internetPrimitive: Bool = true, isArray: Bool = false, isOptional: Bool = false) {
        self.name = name
        self.type = type
    ...
    public static func ==(x: Property, y: Property) -> Bool {
        return x.name == y.name && x.type == y.type && x.internetPrimitive == y.internetPrimitive && x.isArray == y.isArray
    }

    public var hashValue: Int {
        return name.hashValue ^ type.hashValue ^ internetPrimitive.hashValue ^ isArray.hashValue
    }
}
